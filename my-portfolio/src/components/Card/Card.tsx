...
const Card = ({
  ...
  return (
    ...
    >
      <div
        className={
          theme === "dark" ? "gradient-effect-dark" : "gradient-effect-light"
        }
      ></div>
      ...
      <div
        className="d-flex flex-column w-100 position-absolute bottom-0 p-3 cursor-pointer"
        style={{ zIndex: 10, fontSize: ".8rem" }}
      >
        <img src={icon} alt="logo" className="logo mb-2" />
        <h4 style={{ color: themeColors.text }}>{title}</h4>
        <p style={{ color: themeColors.text }}>
          {shortenSentence(description, hovered ? 100 : 50)}
        </p>
        {hovered && (
